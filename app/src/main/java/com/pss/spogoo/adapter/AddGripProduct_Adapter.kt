...
class AddGripProduct_Adapter(
    context: Context,
    arrayListImage: ArrayList<AddGripProductsResponse>
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (myView == null) {
            ...
            myView = mInflater.inflate(R.layout.repairproduct_adapter, parent, false)
        ...
        Glide.with(context).load(arrayListImage.get(position).grip_image)
            .apply(RequestOptions().centerCrop())
            .error(R.drawable.logo)
            .into(holder.productimage!!)

        //Setting name to TextView by position
        holder.productname!!.text = arrayListImage.get(position).grip_name
        holder.mainprice_product!!.text =
            "\u20B9  " + arrayListImage.get(position).regular_price.toString()
        var offerprice = arrayListImage.get(position).special_price
        ...
        if (offerprice == null || offerprice.equals("")) {
            ...
            holder.offerprice_product!!.text =
                "\u20B9  " + arrayListImage.get(position).special_price

            var regularprice = arrayListImage.get(position).regular_price.toString()
            var saledprice = arrayListImage.get(position).special_price.toString()
            var percentage =
...
//
//                val item = CartItem(
//                    arrayListImage.get(position).apparels_id,
...
//                    arrayListImage.get(position).quantity
//                )
...
////                //notify users
////
//                Toast.makeText(context, "item Added apparels", Toast.LENGTH_LONG)
//                    .show()
////
////
